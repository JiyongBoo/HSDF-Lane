...
class GaussianFocalLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        pos_loss = -((1.0 - pred) ** self.alpha) * torch.log(pred) * pos_mask
        neg_loss = -((1.0 - target) ** self.beta) * (pred ** self.alpha) * torch.log(1.0 - pred) * neg_mask
        num_pos = pos_mask.sum().clamp(min=1.0)
        return (pos_loss.sum() + neg_loss.sum()) / num_pos


class SDFFieldLoss(nn.Module):
    """SDF L1 loss + Eikonal (finite-difference) regularization."""

    def __init__(self, eikonal_weight=0.1):
        super().__init__()
        self.l1 = nn.SmoothL1Loss(reduction='none')
        self.eikonal_weight = eikonal_weight

    @torch.cuda.amp.custom_fwd(cast_inputs=torch.float32)
    def forward(self, sdf_pred, z_sampled, gt_heightmap, gt_mask, sdf_valid_mask=None):
        """
        sdf_pred:       (B, max_K, H, W)
        z_sampled:      (B, max_K, H, W)
        gt_heightmap:   (B, 1, H, W)
        gt_mask:        (B, 1, H, W)
        sdf_valid_mask: (1, max_K, H, W)
        """
        sdf_target = z_sampled - gt_heightmap
        mask = gt_mask.expand_as(sdf_pred)
        if sdf_valid_mask is not None:
            mask = mask * sdf_valid_mask

        loss_l1 = (self.l1(sdf_pred, sdf_target) * mask).sum() / mask.sum().clamp(min=1.0)

        dsdf = sdf_pred[:, 1:, :, :] - sdf_pred[:, :-1, :, :]
        dz   = z_sampled[:, 1:, :, :] - z_sampled[:, :-1, :, :]

        # Δz < 1cm guard (near-field numerical instability)
        eikonal_valid = (dz.abs() > 0.01).float()
        gradient = dsdf / dz.clamp(min=1e-4)
        eikonal_error = torch.abs(gradient - 1.0)

        valid_pairs = mask[:, 1:, :, :] * mask[:, :-1, :, :] * eikonal_valid
        loss_eikonal = (eikonal_error * valid_pairs).sum() / valid_pairs.sum().clamp(min=1.0)

        return loss_l1 + self.eikonal_weight * loss_eikonal
