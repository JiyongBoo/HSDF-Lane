"""
train_openlane.py  —  Training script for OpenLane dataset.
==============================================================
...
def train_epoch(rank, model, dataloader, optimizer, scaler, epoch,
    ...
    for idx, batch in enumerate(pbar):
        ...
        (input_data, gt_seg_data, gt_emb_data, offset_y_data, z_data,
         ...
         road2cam, heightmap, cam_w_extrinsics, vehicle_pose,
         *extra) = batch
...
@torch.no_grad()
def validate(rank, world_size, model, val_loader, configs, epoch):
    model.eval()
...
def setup(rank, world_size, master_port='12345'):
...
def worker_function(rank, world_size, config_file, checkpoint_path=None, use_amp=False,
                    wandb_project="hsdflane", wandb_name=None, master_port='12345',
                    skip_val=False, warmup_steps=1000):
    ...
    if is_main_process(rank):
        try:
            ...
            if wandb_key:
                ...
                wandb.login()
            run_name = wandb_name or f"run_{configs.epochs}ep_ddp{world_size}gpu"
            ...
            if resume_run_id:
                ...
                wandb.init(project=wandb_project, name=run_name, group='distributed_training')
    ...
    if optimizer_factory is not None:
        optimizer = optimizer_factory(
            ...
            head_lr=configs.optimizer_params.get('head_lr', configs.optimizer_params.get('lr', 2.5e-4)),
    ...
    if checkpoint_path:
        if getattr(configs, "load_optimizer", True):
            start_epoch, wandb_run_id = resume_training(checkpoint_path, model.module, optimizer, scheduler)
            if is_main_process(rank):
                print(f"Resumed from epoch {start_epoch - 1}, starting training at epoch {start_epoch}")
        else:
    ...
    for epoch in range(start_epoch, configs.epochs):
        ...
        if skip_val:
            ...
            val_results = validate(rank, world_size, model, val_loader, configs, epoch)
...
def main():
    parser = argparse.ArgumentParser(description='HSDF_Lane DDP Training')
    parser.add_argument('--config', type=str,
                        default='./tools/hsdflane_config.py',
    ...
    parser.add_argument('--wandb_project', type=str, default='hsdflane',
