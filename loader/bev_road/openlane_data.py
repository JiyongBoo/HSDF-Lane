...
def _draw_gaussian(heatmap, center_y, center_x, radius, sigma):
    ...
    heatmap_region  = heatmap[center_y - top:center_y + bottom + 1,
                              center_x - left:center_x + right + 1]
    gaussian_region = gaussian[radius - top:radius + bottom + 1,
                               radius - left:radius + right + 1]

    np.maximum(heatmap_region, gaussian_region, out=heatmap_region)


def render_centernet_heatmap(instance_map, sigma=2.0, radius=6):
    """CenterNet-style heatmap from 1-pixel skeleton instance map."""
...
class OpenLane_dataset_with_offset(Dataset):
    ...
    def generate_bev_height_map(self, point_cloud, resolution=(200, 48)):
    ...
    def generate_binary_mask(self, bev_height_map):
        ...
        binary_mask = np.zeros_like(bev_height_map, dtype=np.int32)
        ...
        for idx in res_lane_points_bin:
            ...
            for point in lane_bin:
                ...
                if offset_y is None:  #
                    ipm_image[row, col] = 0
                    continue
                offset_y = np.clip(offset_y, 0.0, 1.0)
                offset_map[row][col] = offset_y
                z_map[row][col] = z

        return ipm_image, offset_map, z_map
    
    def get_seg_offset_for_heightmap(self, idx):
        ...
        bev_heightmap = self.generate_bev_height_map(ipm_roi.T, resolution=(self.ipm_h, self.ipm_w))
    ...
    def get_seg_offset(self, idx, smooth=False):      
        ...
        vehicle_pose = np.array(gt.get('pose', np.eye(4).tolist()))
        cam_w_extrinsics = np.array(gt['extrinsic'])
    ...
    def __getitem__(self, idx):
        image, image_gt, ipm_gt, offset_y_map, z_map, cam_extrinsics, \
            cam_intrinsic, bev_height_map_mask, cam2road, \
            cam_w_extrinsics, vehicle_pose = self.get_seg_offset(idx)
        image_h, image_w, _ = image.shape
        ...
        ipm_gt_instance = torch.tensor(ipm_gt).unsqueeze(0)
        ...
        lane_heatmap_gt = render_centernet_heatmap(
            ipm_gt, sigma=self.heatmap_sigma, radius=self.heatmap_radius)
        lane_heatmap_gt = torch.from_numpy(lane_heatmap_gt).unsqueeze(0)

        return (image,
                ipm_gt_segment.float(), ipm_gt_instance.float(),
                ipm_gt_offset.float(), ipm_gt_z.float(),
                image_gt_segment.float(), image_gt_instance.float(),
                intrinsic.float(), extrinsic.float(), extrinsic_road2cam.float(),
                image_gt_heightmap.float(),
